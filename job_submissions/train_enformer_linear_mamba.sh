...
#BSUB -q gpuqueue
#BSUB -n 8
#BSUB -gpu "num=2:mode=exclusive_process:j_exclusive=yes:gmodel=UnknownNVIDIAA10080GBPCIe"
#BSUB -R "A100 rusage[mem=15]"
#BSUB -R "select[hname!='lc03' && hname!='lc07']"
#BSUB -sla llSC2
#BSUB -W 168:00
#BSUB -o jobs/%J.out
#BSUB -e jobs/%J.err
...
python -m train wandb.group=enformer wandb.name=enformer_mamba_gradbatch_1 experiment=hg38/enformer dataset.batch_size=1 \
 trainer.precision=bf16 dataset.num_workers=8 dataset.rc_aug=false +dataset.mlm=false \
 ...
 model.config.pad_vocab_size_multiple=1 train.pretrained_safetensors_model_path=null \
 trainer.accumulate_grad_batches=8 trainer.devices=2
