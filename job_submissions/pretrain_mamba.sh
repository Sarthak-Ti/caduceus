...
python -m train wandb.group=pretrain wandb.name=mamba_1 experiment=hg38/kmer_pretrain dataset.batch_size=1 \
 trainer.precision=bf16 dataset.num_workers=4 dataset.rc_aug=${RC_AUG} +dataset.mlm=true \
 +dataset.mlm_probability=0.25 model=caduceus model.config.d_model=256 model.config.n_layer=16 model.config.bidirectional=true \
 model.config.bidirectional_strategy=add model.config.bidirectional_weight_tie=true model.config.rcps=true \
 optimizer.lr="${LR}" dataset.max_length=196608 dataset.kmer_len=null \
 model.config.vocab_size=16 model.config.pad_vocab_size_multiple=1 trainer.accumulate_grad_batches=8 \
 +model.config.checkpoint_mixer=true +model.config.checkpoint_mlp=true \
 +dataset.ccre_only=false +dataset.eligible_replacements=[7,8,9,10] task.loss.ignore_index=4
