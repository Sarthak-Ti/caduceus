...
python -m train wandb.group=enformer wandb.name=enformer_kmer_mamba_gradbatches_pretrained_1 experiment=hg38/enformer dataset.batch_size=1 \
 trainer.precision=bf16 dataset.num_workers=4 dataset.rc_aug=false +dataset.mlm=false \
 model=caduceus model.config.d_model=256 model.config.n_layer=10 model.config.bidirectional=true \
 ...
 model.config.vocab_size=15632 model.config.pad_vocab_size_multiple=1 train.pretrained_safetensors_model_path=null \
 trainer.accumulate_grad_batches=8 \
 +model.config.checkpoint_mixer=true +model.config.checkpoint_mlp=true \
 train.pretrained_model_path=/data/leslie/sarthak/caduceus/outputs/2024-09-11/09-10-36-679751/checkpoints/last.ckpt \
#  train.ckpt=/data/leslie/sarthak/caduceus/outputs/2024-09-10/16-15-22-499398/checkpoints/last.ckpt \
