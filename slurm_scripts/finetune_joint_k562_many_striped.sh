...
#SBATCH --job-name=finetune_k562plus10_striped_maskonly_3
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 \
 ...
 +model.config.dt_limit_upper=2.0 task.loss._name_=poisson_loss_nan \
 ...
 \
 +callbacks=model_every_n_steps callbacks.model_checkpoint_every_n_steps.every_n_train_steps=250 trainer.accumulate_grad_batches=32 \
