...
#SBATCH --job-name=joint_cont_sepcnn_combined_gm12878_nomlm_maskonly_pool2
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 +decoder.upsample=2 +encoder.downsample=2 +dataset.mask_only=true dataset.acc_mlm=0.25 dataset.mlm=0 \
