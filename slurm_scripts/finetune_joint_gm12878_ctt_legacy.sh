...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 \
 ...
 train.ckpt="/data1/lesliec/sarthak/caduceus/outputs/2025-11-06/17-30-45-960410/checkpoints/last.ckpt" +train.pretrained_model_state_hook.load_decoder=true \
#  train.pretrained_model_path="/data1/lesliec/sarthak/caduceus/outputs/2025-05-15/17-49-16-937449/checkpoints/last.ckpt"
...
#  trainer.precision=bf16 dataset.num_workers=4 loader.num_workers=4 model.config.vocab_size=1 model.config.pad_vocab_size_multiple=1 \
#  \
#  model=caduceus model.config.d_model=256 model.config.n_layer=16 model.config.bidirectional=true \
#  model._name_=dna_embedding_caduceus model.config.bidirectional_strategy=add model.config.bidirectional_weight_tie=true model.config.rcps=false \
#  optimizer.lr="1e-4" \
...
#  +decoder.d_model=256 +decoder.d_output=1 +dataset.additional_data=/data1/lesliec/sarthak/data/enformer/data/labels.zarr \
#  +dataset.additional_data_idxs=/data1/lesliec/sarthak/data/DK_zarr/idx_lists/all_matched_immune_CAGE.json \
#  +dataset.data_idxs=/data1/lesliec/sarthak/data/DK_zarr/idx_lists/all_matched_immune.json trainer.limit_train_batches=0.00001 trainer.limit_val_batches=0.001 +train.remove_test_loader_in_eval=true
#  train.ckpt="/data1/lesliec/sarthak/caduceus/outputs/2025-04-17/12-31-41-192495/checkpoints/last.ckpt" +train.pretrained_model_state_hook.load_decoder=true