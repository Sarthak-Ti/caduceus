...
#SBATCH --job-name=joint_cont_sepcnn_combined_nobcell
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 dataset.load_in=false +dataset.data_idxs=/data1/lesliec/sarthak/data/DK_zarr/idx_lists/nob_immune.json \
 \
