...
#SBATCH --job-name=joint_nomlm_maskonly_DE_all_multitask_pool
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 +dataset.mask_only=true dataset.acc_mlm=0.25 dataset.mlm=0 +dataset.mask_tie=0.999 +dataset.multitasking=true dataset.rc_aug=true +dataset.independent_tracks=true \
 decoder.d_output2=14 encoder.d_input2=28 \
#  +decoder.upsample=8 +encoder.downsample=8
