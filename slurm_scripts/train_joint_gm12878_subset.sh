...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 \
 +model.config.skip_embedding=true trainer.devices=$NUM_GPUS trainer.limit_train_batches=0.1 trainer.limit_val_batches=0.1 \
...
CUDA_VISIBLE_DEVICES=0 pixi run python -m train wandb=null experiment=hg38/joint_pretrain dataset.batch_size=1 \
 trainer.precision=bf16 dataset.num_workers=5 loader.num_workers=5 model.config.vocab_size=1 model.config.pad_vocab_size_multiple=1 \
 \
 model=caduceus model.config.d_model=256 model.config.n_layer=16 model.config.bidirectional=true \
 model._name_=dna_embedding_caduceus model.config.bidirectional_strategy=add model.config.bidirectional_weight_tie=true model.config.rcps=false \
 optimizer.lr="1e-3" \
 ...
 dataset.load_in=false \
 \
 +model.config.skip_embedding=true trainer.devices=1 trainer.limit_train_batches=0.001 trainer.limit_val_batches=0.01 \