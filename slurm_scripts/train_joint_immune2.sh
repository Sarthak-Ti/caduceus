...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 optimizer.lr="1e-3" +train.remove_test_loader_in_eval=true train.interval=epoch \
 \
 ...
 dataset.load_in=false +dataset.data_idxs=/data1/lesliec/sarthak/data/DK_zarr/idx_lists/all_matched_immune.json trainer.accumulate_grad_batches=8 \
 \
 +model.config.skip_embedding=true trainer.devices=$NUM_GPUS \
 +dataset.mask_only=true dataset.acc_mlm=0.25 dataset.mlm=0 +encoder.ctt=true +dataset.return_celltype_idx_og=true \
 train.ckpt=/data1/lesliec/sarthak/caduceus/outputs/2025-10-31/13-49-05-409924/checkpoints/last.ckpt +train.pretrained_model_state_hook.load_decoder=true
