...
#SBATCH --job-name=joint_nomlm_maskonly_allcelltypes_multitask_pool_3
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 \
 dataset.data_path=/data1/lesliec/sarthak/data/DK_zarr/zarr_arrays/dnase_chunkchrom_reprocessed.zarr dataset.batch_size_eval=1 \
 dataset.load_in=false scheduler=step trainer.accumulate_grad_batches=16 +dataset.sequences_bed_file=/data1/lesliec/sarthak/data/DK_zarr/sequences_enformer.bed \
 \
 +model.config.skip_embedding=true \
 +dataset.mask_only=true dataset.acc_mlm=0.25 dataset.mlm=0 +decoder.upsample=8 +encoder.downsample=8 +dataset.mask_tie=0.95 +dataset.multitasking=true dataset.rc_aug=true \
 decoder.d_output2=674 encoder.d_input2=1348 \
 train.ckpt="/data1/lesliec/sarthak/caduceus/outputs/2025-12-01/14-32-41-762835/checkpoints/last.ckpt" +train.pretrained_model_state_hook.load_decoder=true
...
#  dataset.load_in=false scheduler=step trainer.accumulate_grad_batches=16 \
#  \
#  +model.config.skip_embedding=true \
#  +dataset.mask_only=true dataset.acc_mlm=0.25 dataset.mlm=0 +decoder.upsample=8 +encoder.downsample=8 +dataset.mask_tie=0.99 +dataset.multitasking=true dataset.rc_aug=true \
#  decoder.d_output2=674 encoder.d_input2=1348 \
