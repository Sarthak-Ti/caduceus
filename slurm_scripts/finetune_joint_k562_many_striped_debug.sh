...
#SBATCH --job-name=finetune_k562plus10_striped_maskonly_debug4
...
pixi run srun python -m train_debug wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 \
 ...
 +model.config.dt_limit_upper=2.0 +optimizer.eps=1e-6 +decoder.log=true \
 optimizer.lr="3e-4" +train.remove_test_loader_in_eval=true \
 \
 scheduler._name_=cosine_warmup_timm +scheduler.t_initial=30000 +scheduler.warmup_t=500 +scheduler.lr_min=1e-4 +scheduler.t_in_epochs=false train.interval=step +model.config.a_log_clamp_min=-8 +model.config.monitor_a_log=true \
 \
...
# python -m train_debug wandb=null experiment=hg38/joint_finetune dataset.batch_size=1 \
...
#  \
#  scheduler._name_=cosine_warmup_timm +scheduler.t_initial=30000 +scheduler.warmup_t=500 +scheduler.lr_min=1e-4 \
#  \
#  dataset.acc_type=continuous +dataset.crop_additional=524288 dataset.shift_sequences=1000 \
#  \
#  dataset.data_path=/data1/lesliec/sarthak/data/DK_zarr/zarr_arrays/dnase_chunkchrom_processed.zarr dataset.length=2097152 +dataset.return_celltype_idx_og=true \
