...
#SBATCH --job-name=joint_k562_striped_maskonly
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 \
 dataset.data_path=/data1/lesliec/sarthak/data/DK_zarr/zarr_arrays/cell_type_arrays/K562_DNase.npz dataset.acc_mlm=0.25 dataset.mlm=0 +dataset.mask_only=true \
 ...
 train.ckpt=/data1/lesliec/sarthak/caduceus/outputs/2026-04-09/20-50-29-145347/checkpoints/last.ckpt +train.pretrained_model_state_hook.load_decoder=true \
