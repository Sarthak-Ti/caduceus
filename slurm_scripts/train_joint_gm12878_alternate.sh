...
#SBATCH --job-name=joint_cont_onecnn_gm12878
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_pretrain dataset.batch_size=1 \
 ...
 optimizer.lr="1e-3" \
 \
 train.task2=reg train.custom_metric=poisson_loss_mask dataset.acc_type=continuous encoder.joint=true \
 \
...
#  optimizer.lr="1e-3" \
#  \
#  train.task2=mlm train.custom_metric=ce_loss_mask_acc dataset.acc_type=category \
#  \
#  dataset.data_path=/data1/lesliec/sarthak/data/DK_zarr/zarr_arrays/cell_type_arrays/GM12878_DNase.npz dataset.data_is_zarr=false \
#  dataset.load_in=false \
#  \
#  +model.config.skip_embedding=true trainer.devices=2 \