...
#SBATCH --job-name=joint_cont_sepcnn_combined_fullenformer
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 \
 ...
 \
 dataset.data_path=/data1/lesliec/sarthak/data/DK_zarr/zarr_arrays/cell_type_arrays/GM12878_DNase.npz dataset.acc_mlm=1 +dataset.mask_only=true \
 +dataset.sequences_bed_file=/data1/lesliec/sarthak/data/DK_zarr/sequences_enformer.bed \
 dataset.load_in=false +dataset.additional_data=/data1/lesliec/sarthak/data/enformer/data/labels.zarr \
 \
 ...
 +decoder.d_model=256 +decoder.d_output=5313 \
 train.ckpt=/data1/lesliec/sarthak/caduceus/outputs/2025-06-09/15-24-20-013709/checkpoints/last.ckpt +train.pretrained_model_state_hook.load_decoder=true
