...
#SBATCH --job-name=joint_cont_sepcnn_combined_gm12878only_finetune_phastcons100_bin
...
pixi run srun python -m train wandb.group=joint_pretrain wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 trainer.devices=$NUM_GPUS \
...
CUDA_VISIBLE_DEVICES=0 python -m train wandb=null experiment=hg38/joint_finetune dataset.batch_size=1 trainer.devices=1 \
