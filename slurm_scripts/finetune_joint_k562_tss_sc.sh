...
#SBATCH --job-name=k562_tss_sc_tssmask
...
pixi run srun python -m train wandb.group=tss_finetune wandb.name=$SLURM_JOB_NAME experiment=hg38/joint_finetune dataset.batch_size=1 \
 ...
 \
 decoder._name_=tss +decoder.d_output=1 +decoder.hidden_dim=128 trainer.accumulate_grad_batches=16 +dataset.append_tss_mask=true +decoder.pool_region=all \
