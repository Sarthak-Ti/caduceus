...
def mlm_getitem(seq, mlm_probability=0.15, contains_eos=False, tokenizer=None, num_tokens=None, eligible_replacements=None):
    ...
    target[~masked_indices] = tokenizer.pad_token_id  # We only compute loss on masked tokens, so this ignores the pad token id
    ...
    if eligible_replacements is not None:
        ...
        random_words = torch.randint(num_tokens, size=target.shape, dtype=torch.long)
