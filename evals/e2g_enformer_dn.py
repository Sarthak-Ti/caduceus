...
#variant of e2g_enformer.py that can also dinucleotide shuffle the element instead of, or as well as,
#scaling down its accessibility. See --perturbation.
print('E2G enformer evaluation (dinucleotide shuffle)', flush=True)
...
def main(args):
    ...
    #for --perturbation acc this is n_alt=1, i.e. the original (N, 2, 896) layout
    n_alt = args.n_shuffles if args.perturbation in ('shuffle', 'both') else 1

    #by default collapse the shuffles to their mean, so the output stays (N, 2, 896) like acc mode.
    #--save_all_shuffles keeps every individual shuffle instead, giving (N, 1+n_shuffles, 896)
    average_shuffles = args.perturbation in ('shuffle', 'both') and not args.save_all_shuffles
    n_out = 2 if average_shuffles else 1 + n_alt

    outputs = np.zeros((len(gs_df), n_out, 896))
    ...
    shuffle_std = np.zeros((len(gs_df), 896)) if average_shuffles else None
    ...
    for i, row in tqdm(gs_df.iterrows(), total=len(gs_df)):
        ...
        if in_context[i]:
            ...
            if args.perturbation in ('acc', 'both'):
                startmask = max(0, e_start - args.dist_additional_mask)
                endmask   = min(SEQ_LEN, e_end + args.dist_additional_mask)
            ...
            if args.perturbation in ('shuffle', 'both'):
                sh_start = max(0, e_start - args.dist_additional_shuffle)
                sh_end   = min(SEQ_LEN, e_end + args.dist_additional_shuffle)
        ...
        for j in range(0, 1 + n_alt, args.batch_size):
            ...
            outs.append(o.cpu().numpy().reshape(o.shape[0], -1)) #not squeeze, a chunk of size 1 would collapse it
        preds = np.concatenate(outs, axis=0) #(1+n_alt) x 896, row 0 is the reference

        if average_shuffles:
            outputs[i] = np.stack([preds[0], preds[1:].mean(axis=0)])
            shuffle_std[i] = preds[1:].std(axis=0)
        else:
            outputs[i] = preds
    ...
    with open(meta_path, 'w') as f:
        json.dump({**vars(args), 'n_alt': n_alt, 'averaged': average_shuffles,
                   'shape': list(outputs.shape)}, f, indent=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_path', type=str,
                        default='/data1/lesliec/sarthak/caduceus/outputs/2025-08-15/12-23-01-318424/checkpoints/last.ckpt',
                        help='Path to the model checkpoint')
    parser.add_argument('-o', '--output', type=str, default='k562_unstranded_enformer',
    ...
    parser.add_argument('--perturbation', type=str, default='acc', choices=['acc', 'shuffle', 'both'],
                        help='acc: scale down accessibility over the element (original behaviour). '
                             ...
                             'both: apply both to every alternate row.')
    ...
    parser.add_argument('--save_all_shuffles', action='store_true',
                        help='Save every individual shuffle instead of their mean. Default is to average '
                             'the shuffles, giving (N, 2, 896) with row 1 = mean over shuffles.')
