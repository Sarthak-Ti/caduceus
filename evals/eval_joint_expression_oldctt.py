...
class Evals():
    ...
    def __call__(self, idx=None, data=None, ctt=0):
        #now evaluate the model on one example
        if data is None:
            (seq,acc,t),(seq_unmask,acc_unmask,exp) = self.dataset[idx]
            
            x = seq.unsqueeze(0)
            y = acc.unsqueeze(0)
        else:
            (x,y,t),(seq_unmask,acc_unmask,exp) = data

            if x.dim() == 2:
                x = x.unsqueeze(0) #add batch dim
                y = y.unsqueeze(0) #add batch dim
        
        t = torch.tensor([ctt]) #add batch dim, if ctt is given use that instead of the one in the data
        x,y = x.to(self.device), y.to(self.device)
        t = t.to(self.device)
        # print(t.shape)
        
        with torch.no_grad():
            x1 = self.encoder(x,y,t)
...
ckpt_paths = ['/data1/lesliec/sarthak/caduceus/outputs/2026-05-07/12-23-37-459297/checkpoints/last.ckpt']
model_names = ['immune_CAGE_CTT']
...
for ckpt_path, model_name in zip(ckpt_paths, model_names):
    ...
    evals = Evals(ckpt_path, load_data=False, sequences_bed_file='/data1/lesliec/sarthak/data/DK_zarr/sequences_enformer.bed',
                  ...
                  additional_data_idxs=[5110]) #, additional_data_idxs=[5110]) #the 5110 is for if additional data covers all cell types but just want one
