...
class Evals():
    ...
    def mask(self, start=None, stop=None, idx=None, data=None, run=True,
        ...
        numtracks = acc.shape[0]
        #set the second half of values to zero, as those are the mask indicators
        acc[numtracks//2:] = 0
        # acc[-1] = 0
        ...
        if mask_accessibility:
            acc[numtracks//2:, start_acc:stop_acc] = 1 #tell the model it's masked
            acc[:numtracks//2, start_acc:stop_acc] = 0 #zero out the original accessibility values
            # acc[-1, start_acc:stop_acc] = 1 #tells the model it's masked
            # acc[:-1, start_acc:stop_acc] = 0 #zero out the original accessibility values
        ...
        if run:
            ...
            seq_out, acc_out, seq_unmask_out, acc_unmask_out = self(data=data, ctt_val=ctt_val)
