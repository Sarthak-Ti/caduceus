...
#Same experiment as e2g_tss.py -- knock down accessibility around a CRISPR element and read the
#change in predicted expression -- but the loader here builds the decoder from the config's
#_name_ instead of hardcoding TSSDecoder, and collapses the (profile, counts) tuple to counts.
print('E2G for centering at TSS model (profile decoder)', flush=True)
...
class Evals():
    def __init__(self,
        ...
        #build the decoder the config actually asks for instead of hardcoding one class:
        #'tss' -> TSSDecoder (single scalar), 'tss_profile' -> TSSProfileDecoder (profile + counts).
        #the two take different kwargs (pool_region/d_output vs n_tracks/profile_region/count_region),
        #so hardcoding either one TypeErrors on the other's checkpoints
    ...
    def __call__(self, idx=None, data=None, softplus=False, og=False, ctt_val=None, return_profile=False):
        ...
        with torch.no_grad():
            ...
            out = self.decoder(x1, mask=tss_mask, gene_mask=gene_mask)

        #TSSProfileDecoder returns (profile, counts): profile is (batch, n_tracks, length) shape
        #logits, counts is (batch, n_tracks) on the log target scale -- the same quantity a
        #single-output TSSDecoder returns. Collapse to counts so the scalar eval paths are
        #identical across the two decoders; ask for return_profile if you want both.
        if isinstance(out, tuple):
            profile, counts = out
            return (profile, counts) if return_profile else counts
        return out
...
def main(args):
    ...
    for i, row in tqdm(gs_df.iterrows(), total=len(gs_df)):
        ...
        #counts is (2, n_tracks) = (2, 1) here, so squeeze gives (2,): row 0 unperturbed, row 1 perturbed
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run E2G evaluation for a two-output (profile+count) TSS model')
    parser.add_argument('--ckpt_path', type=str,
                        default='/data1/lesliec/sarthak/caduceus/outputs/2026-08-05/12-48-43-803485/checkpoints/31-val_loss=1.47817.ckpt',
                        help='Path to the model checkpoint')
    parser.add_argument('-o', '--output', type=str, default='k562_tss_sc_rna_profile2_ep31',
