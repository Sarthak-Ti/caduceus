...
class Evals():
    ...
    def __call__(self, idx=None, data=None, softplus=True, og=False, ctt_val=0):
        ...
        ctt = ctt.squeeze() #make it just batch size
...
def main(args):
    ...
    for i in tqdm(range(qtls.shape[0])):
        ...
        ctt = ctt.unsqueeze(0).repeat(2, 1, 1)
        ...
        data = ((s,a,ctt),(None,None))
        
        out = evals(data = data, ctt_val=args.ctt_val) #now get the output
